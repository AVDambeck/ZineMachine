...
5. render front to FRONT.pdf;
6. render back to BACK.pdf;
...
#5 render front to FONT.pdf;
logging.info(f'begining render')
html_path = os.path.abspath('../html/accordian_front.html')
converter.convert(f'file:///{html_path}', 'FRONT.pdf')

#6 render back to BACK.pdf;
html_path = os.path.abspath('../html/accordian_back.html')
converter.convert(f'file:///{html_path}', 'BACK.pdf')

#7 join pdfs. output to PRINT.pdf;
subprocess.run(f"bash join-front-back.sh", shell=True)
